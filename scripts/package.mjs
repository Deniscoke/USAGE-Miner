...
function writeManifest(files) {
  ...
  //
  // Behind a flag, though. An ordinary `npm run package` -- a check, a test, a
  // look at whether it still builds -- must not silently repoint the live
  // download page at bytes nobody published. The installer wrapper is not
  // reproducible, so every such build produces a different hash, and the page
  // would start advertising a checksum that matches nothing on the release.
  // Overwriting it is part of cutting a release, so it takes saying so.
  if (!publishing) {
    log("manifest unchanged (pass --publish when cutting a release)");
    return manifest;
  }
